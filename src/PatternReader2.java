...
/**
 * @version 2.0 -- 09/04/013 -22:24hrs
 * @author Ewerton Lopes
 * 
 *         This class recognize a method running time between a given string
 *         pattern or by the "Time: ", "OK", "FAILURES" words in a given file
 *         (generally text files). To create an object PatternReader2 and thus
 *         make calls to its method, we must declare directory names and file
 *         extension on the constructor. This program generates an output
...
public class PatternReader2 {
	...
	private String fileExtensionNameForReading;

	public PatternReader2() {
		this.currentDirectory = new File(System.getProperty("user.dir"));
		this.persistenceController = new Persistence(
				System.getProperty("user.dir") + "\\TimeReaderResults");
		this.fileExtensionNameForReading = ".txt";
	}

	public PatternReader2(String directoryPath,
			String persistenceDirectoryPath, String fileExtension) {
		this.currentDirectory = new File(directoryPath);
		this.persistenceController = new Persistence(persistenceDirectoryPath);
		this.fileExtensionNameForReading = fileExtension;
	}

	public static void main(String[] args) {
		PatternReader2 timeReader = new PatternReader2(
				"C:\\Users\\Ewerton Lopes\\Desktop\\git\\Testes",
				"C:\\Users\\Ewerton Lopes\\Desktop\\git\\Testes\\Results",
				".txt"); // Create a timeReader Object
		timeReader.ListTheFilesOnDirectory(); // Reading the files on the
		// directory
		timeReader.SearchPatternByEachLine(); // Classifies files by "TIME",
												// "OK" and "FAILURES" words
			...
			this.filesRead = this.currentDirectory.listFiles(new FileFilter() {
				public boolean accept(File pathname) {
					return pathname.getName().toLowerCase()
							.endsWith(fileExtensionNameForReading);
				}
			});
	...
	private void SearchPatternByEachLine() {
		for (File file : this.filesRead) {
			...
			try {
				...
				// from null. The readLine () method returns the row position in
				// the
				...
				while ((line = bufferedReader.readLine()) != null) {
					...
					if (line.contains("OK")) {
						this.persistenceController.CreateNewFileForTheResults(
								this.currentDirectory.getName()
										+ "_JUNITResults.txt", "OK", false);
					} else if (line.contains("FAILURES")) {
						this.persistenceController.CreateNewFileForTheResults(
								this.currentDirectory.getName()
										+ "_JUNITResults.txt", "FAILURES",
								false);
					}
				}
